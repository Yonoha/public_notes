...
\newcommand{\vbbSigma}{\mathbbl{\Sigma}^{!}}
...
\title{Notes on Six-functor formalisms (Under Construction)}
...
The aim of this note is to explain \textit{what a six-functor formalism is} and summarize the basic results and examples of six-functor formalisms.
A six-functor formalism is a framework to formalize a cohomology theory, which provide a structured and systematic approach to handling cohomological operations. 
...
We will identify $1$-categories, quasi-categories, and complete Segal anima using the nerve and Rezk nerve constructions.  
From this point forward, all categories are assumed to be $\infty$-categories, specifically complete Segal anima.

In \cref{sec.category_of_correspondences}, we let
\begin{itemize}
  \item $\Cat$ denote the category of small categories.
  \item $\Corr(\C,\C_0)$ denote the category of correspondences for a geometric setup $(\C,\C_0)$.
  \item $\GS$ denote the category of geometric setups.
\end{itemize}
...
\begin{itemize}
  ...
  \end{align*}
\end{itemize}
Moreover, there exists a natural map 
\begin{align*}
...
\begin{align*}
  \C^{\myop} \to \Corr(\C,E) 
  &: (Y \xrightarrow{f^{\myop}} X) \mapsto (Y \xleftarrow{f} X \xrightarrow{\id_{X}} X), \\
  E \to \Corr(\C,E) 
  &: (X \xrightarrow{f} Y) \mapsto (X \xleftarrow{\id_{X}} X \xrightarrow{f} Y).
\end{align*} 
...
\begin{align*}
  \rmD : \Corr(\C,E) \to \Cat.
\end{align*}
To define a six-functor formalism, we don't need any extra data.
That is, a six-functor formalism is a special case of a three-functor formalism, where three-functors $\otimes$, $f^{*}$ and $f_{!}$ admit right adjoint functors.  
...
\begin{definition}[\cite{HM24} Definition 2.1.1] \label{HM24.def.2.1.1}
  ...
  \begin{enumerate}
    ...
    \item The subcategory $\C_0$ is closed under pullbacks along morphisms in $\C$. 
    \item The subcategory $\C_0$ admits and the inclusion $\C_0 \subseteq \C$ preserves pullbacks.
  \end{enumerate}
\end{definition}

\begin{definition}
  Let $(\C,\C_0)$ and $(\D,\D_0)$ be geometric setups.
  A \textit{morphism of geometric setups} from $(\C,\C_0)$ to $(\D,\D_0)$ is a functor $f : \C \to \D$ which preserves pullbacks of the form $X \xrightarrow{f} Y \xleftarrow{g} Z$ in $\C$ where $f$ lies in $\C_0$.
  We let $\Fun^{\mathrm{GS}}(\C,\D)$ denote the full subcategory of $\Fun(\C,\D)$ spanned by morphisms of geometric setups.
\end{definition}

\begin{example} \label{HM24.eg.2.1.4} % [\cite{HM24} Example 2.1.4] 
  Let $\C$ be a category.
  \begin{enumerate}
    \item If $\C$ admits pullbacks, then the pair $\C^{\sharp} := (\C,\C)$ is a geometric setup.
    \item The pair $\C^{\flat} := (\C^{\myop},\core\C)$ is a geometric setup.
    \item Let $(\C,\C_0)$ be a geometric setup. 
    Then the inclusion $\C_0 \subseteq \C$ induces a morphism of geometric setups $\C_0^{\sharp} \to (\C,\C_0)$.
    \item Let $(\C,\C_0)$ be a geometric setup.
    Then the identity $\C \subseteq \C$ induces a morphism of geometric setups $\C^{\flat} \to (\C,\C_0)$.
  \end{enumerate}
\end{example}

\begin{proof}
  (1) and (3) are clear. % (1) since the inclusion $\C \subseteq \C$ is an identity functor
  (2) and (4) follow from the fact that the pullback of isomorphisms is also an isomorphism.
\end{proof}
...
\begin{definition}
  ...
  \begin{itemize}
    ...
    \item For every pair of objects $(\C,\C_0)$ and $(\D,\D_0)$ of $\GS^{\Delta}$, the mapping space 
    \begin{align*}
      \Map_{\GS^{\Delta}}((\C,\C_0),(\D,\D_0)) 
      := \core\Fun^{\mathrm{GS}}(\C,\D).
    \end{align*}
  \end{itemize}
  Then the simplicial nerve of $\GS^{\Delta}$ is a category by \cite[Proposition 1.1.5.10]{HTT}.
  We let $\GS$ denote the simplicial nerve $\N_{\Delta}(\GS^{\Delta})$.
  We will refer to $\GS$ as the \textit{category of geometric setups}.
\end{definition}

\begin{lemma}[\cite{HM24} Lemma 2.1.6] \label{HM24.lem.2.1.6}
  The category $\GS$ admits small limits.
\end{lemma}

\begin{proof}
  Let $I \to \GS : i \mapsto (\C_i,(\C_0)_i)$ be a small diagram.
  We show that a pair $(\lim_{i}\C_i, \lim_{i}(\C_0)_i)$ is a limit in $\GS$.
\end{proof}

% \begin{remark} \label{HM24.rem.2.1.2} % [\cite{HM24} Remark 2.1.2]
%   We can identity a geometric setup $(\C,\C_0)$ with a pair $(\C,E)$ where $E$ is a class of morphisms in $\C$ satisfying the following conditions:
%   \begin{enumerate}
%     \item The class $E$ contains all equivalences, is closed under composition and pullbacks along morphisms in $\C$.
...
\begin{definition}[\cite{HM24} Definition 2.2.1] \label{HM24.def.2.2.1}
  For every $n \geq 0$, we let 
  \begin{enumerate}
    \item $\bbSigma_n$ denote the poset of pairs $(i,j)$ with $0 \leq i \leq j \leq n$, where $(i,j) \leq (i',j')$ if and only if $i \leq i'$ and $j' \leq j$.
    \item $\vbbSigma_n$ denote the subposet of $\bbSigma_n$, where $(i,j) \leq (i',j')$ if and only if $i \leq i'$ and $j'=j$.
  \end{enumerate}
\end{definition}
...
%   \begin{enumerate}
%     \item We define the simplicial set $\bbSigma_n$ by $\bbSigma_n_k := \Hom_{\mathrm{sSet}}([k] \star [k]^{\myop},[n])$
%     % \begin{align*}
%     %   \bbSigma_n_k := \Hom_{\mathrm{sSet}}([k] \star [k]^{\myop},[n])
%     % \end{align*}
%     for every $k \geq 0$.
%     \item We let $\vbbSigma_n$ denote the subsimplicial set of $\bbSigma_n$ satisfying the following condition: 
%     Its projection to the second component is a degenerate edge.
%     \item We let $\bbLambda^n$ denote the subsimplicial set of $\bbSigma_n$ spanned by the image of objects $(i,j)$ of $[-] \star [-]^{\myop}$ satisfying $j \leq i+1$.
%   \end{enumerate}
% \end{notation}

% \begin{remark}
%   The simplicial set $\bbSigma_n$ is an example of twisted arrow categories (see \cite{HA} Construction 5.2.1.1).
%   By \cite[Proposition 5.2.1.3]{HA}, $\bbSigma_n$ is a category.
%   Then $\vbbSigma_n$ and $\bbLambda^n$ are full subcategories of $\bbSigma_n$.
% \end{remark}

\begin{remark}
  % The constructions $[n] \mapsto \bbSigma_n$ and $[n] \mapsto \vbbSigma_n$ define functors
  % \begin{align*}
  %   \bbSigma : \prism \to \Cat 
  %   \quad \text{ and } \quad 
  %   \vbbSigma : \prism \to \Cat.
  % \end{align*}
  For every $n \geq 0$, the pair $(\bbSigma_n, \vbbSigma_n)$ is a geometric setup by definition.
  Thus the construction $[n] \mapsto (\bbSigma_n, \vbbSigma_n)$ defines a cosimplicial object 
  \begin{align*}
...
\begin{definition}[\cite{HM24} Definition 2.2.10] \label{HM24.def.2.2.10}
  ...
  We define a functor 
  \begin{align*}
    \Corr(\C,\C_0) : \prism^{\myop} \to \An
  \end{align*}
  by 
  \begin{align*}
    \Corr(\C,\C_0)_n := \Map_{\GS}((\bbSigma_n, \vbbSigma_n), (\C,\C_0)) 
  \end{align*}
  for every $n \geq 0$.
  ...
  % \end{align*}
  We will refer to it as the \textit{category of correspondences}.
\end{definition}

\begin{remark}
  The construction $(\C,\C_0) \mapsto \Corr(\C,\C_0)$ defines a functor
  \begin{align*}
...
\begin{proposition}[\cite{HM24} Proposition 2.2.9] \label{HM24.prop.2.2.9}
  Let $(\C,\C_0)$ be a geometric setup.
  Then the category of correspondences $\Corr(\C,\C_0) : \prism^{\myop} \to \An$ is a complete Segal anima.
  Thus the functor $\Corr : \GS \to \Fun(\prism^{\myop},\An)$ factors through the full subcategory of complete Segal anima.
\end{proposition}

\begin{lemma}[\cite{HM24} Lemma 2.2.11] \label{HM24.lem.2.2.11}
  The functor $\Corr : \GS \to \Fun(\prism^{\myop},\An)$ preserves small limits.
\end{lemma}

\begin{proof}
  Let $I \to \GS : i \mapsto (\C_i,(\C_0)_i)$ be a small diagram.
  We show that a natural transformation 
  \begin{align*}
    \Corr(\lim_{i}\C_i, \lim_{i}(\C_0)_i) 
    \to \lim_{i}\Corr(\C_i,(\C_0)_i)
  \end{align*}
  is an isomorphism.
  Since the limits in $\Fun(\prism^{\myop},\An)$ can be calculated pointwise, it suffices to show that a morphism of anima
  \begin{align*}
    \Corr(\lim_{i}\C_i, \lim_{i}(\C_0)_i)_n 
    \to \lim_{i}\Corr(\C_i,(\C_0)_i)_n
  \end{align*}
  is an isomorphism for every $n \geq 0$.
  It follows from the fact the functor $\Map_{\GS}((\bbSigma_n,\vbbSigma_n), -)$ commutes with limits for every $n \geq 0$.
\end{proof}
...
% \begin{normaltext}
%   The inclusion $\C_0 \to \Corr(\C,\C_0) : (f : X \to Y) \mapsto (X \xleftarrow{\id_X} X \to Y)$ can be described as follows:
%   \begin{enumerate}
%     \item A morphism $f : X \to Y$ in $\C_0$ is a functor $[1] \to \C_0$ and a morphism $X \xleftarrow{\id_X} X \to Y$ in $\Corr(\C,\C_0)$ is a functor $(\bbSigma^1,\vbbSigma^1) \to (\C,\C_0)$.
%     \item A natural transformation $[1] \to \C_0$ to $(\bbSigma^1,\vbbSigma^1) \to (\C,\C_0)$ can be obtained by the composition 
%     \begin{align*}
%       ([1] \to \C_0) 
%       \to ((\bbSigma^1,\vbbSigma^1) \to (\C_0,\C_0)) 
%       \to ((\bbSigma^1,\vbbSigma^1) \to (\C,\C_0)),
%     \end{align*}
%     where the first morphism is induced by the projection $(\bbSigma^1,\vbbSigma^1) \to [1] : (i,j) \mapsto i$, and the second is induced by the inclusion $\C_0 \subseteq \C_0$.
%   \end{enumerate} 
% \end{normaltext}

\subsection{Two Inclusions to \texorpdfstring{$\Corr(\C,\C_0)$}{Corr(C,C_0)}}

In this section, we construct two inclusions $\C_0 \to \Corr(\C,\C_0)$ and $\C^{\myop} \to \Corr(\C,\C_0)$.

\begin{lemma}
  \begin{enumerate}
    \item For every $n \geq 0$, the first-component projection $(\bbSigma_n,\vbbSigma_n) \to [n]^{\sharp}$ is a morphism of geometric setups.
    \item For every $n \geq 0$, the second-component projection $(\bbSigma_n,\vbbSigma_n) \to [n]^{\myop,\flat}$ is a morphism of geometric setups.
  \end{enumerate}
  Moreover, these constructions define morphisms of cosimplicial objects $(\bbSigma,\vbbSigma) \to [-]^{\sharp}$ and $(\bbSigma,\vbbSigma) \to [-]^{\myop,\flat}$.
\end{lemma}

\begin{lemma}
  \textcolor{red}{not sure.}
  For every $n \geq 0$, the second-component projection $(\bbSigma_n,\vbbSigma_n) \to [n]^{\myop,\flat}$ is an isomorphism of geometric setups.
\end{lemma}

\begin{remark} \label{HM24.rem.2.2.12} % [\cite{HM24} Remark 2.2.12] 
  Let $(\C,\C_0)$ be a geometric setup. 
  Then the composition
  \begin{align*}
    \Map_{\Cat}([n],\C_0) 
    \simeq \Map_{\GS}([n]^{\sharp}, \C_0^{\sharp}) 
    \to \Map_{\GS}((\bbSigma_n,\vbbSigma_n), \C_0^{\sharp})
    \to \Map_{\GS}((\bbSigma_n,\vbbSigma_n), (\C,\C_0))
  \end{align*}
  defines a morphism of anima
  \begin{align*}
    (\C_0)_n \to \Corr(\C,\C_0)_n.
  \end{align*}
  Since all constructions are functorial, it determines a morphism of cosimplicial objects
  \begin{align*}
    \C_0 \to \Corr(\C,\C_0).
  \end{align*}
\end{remark}
...
\begin{remark}
  ...
  \begin{align*}
    \Map_{\Cat}([n]^{\myop},\C) 
    \simeq \Map_{\GS}([n]^{\myop,\flat}, \C^{\flat}) 
    \to \Map_{\GS}((\bbSigma_n,\vbbSigma_n), C^{\flat})
    \to \Map_{\GS}((\bbSigma_n,\vbbSigma_n), (\C,\C_0))
  \end{align*}
  defines a morphism of anima
  \begin{align*}
    (\C^{\myop})_n \to \Corr(\C,\C_0)_n.
  \end{align*}
  Since all constructions are functorial, it determines a morphism of cosimplicial objects
  \begin{align*}
    \C^{\myop} \to \Corr(\C,\C_0).
...
\begin{lemma}[\cite{HM24} Lemma 2.3.1] \label{HM24.lem.2.3.1}
  ...
  \begin{align*}
    \C_0^{-} 
    := (\C_0^{\myop})^{\coprod,\myop} \times_{\Fin_{*}^{\myop}} \core\Fin_{*}.
  \end{align*}
...
\begin{proposition}[\cite{HM24} Proposition 2.3.3] \label{HM24.prop.2.3.3}
  Let $(\C,\C_0)$ be a geometric setup.
  We denote 
  \begin{align*}
...
\begin{proposition}[\cite{HM24} Proposition 2.3.5] \label{HM24.prop.2.3.5}
  The functor $(\C,\C_0) \to \Corr(\C,\C_0)^{\otimes}$ enhances to a functor 
  \begin{align*}
    \Corr^{\otimes} : \GS \to \Op.
  \end{align*}
\end{proposition}

\begin{proposition}[\cite{HM24} Proposition 2.3.7] \label{HM24.prop.2.3.7}
  Let $(\C,\C_0)$ be a geometric setup.
  The functor $\C^{\myop} \to \Corr(\C,\C_0)$ enhances to a functor 
  \begin{align*}
    \C^{\myop,\coprod} \to \Corr(\C,\C_0)^{\otimes}.
  \end{align*}
\end{proposition}

\subsection{The Symmetric Monoidal Structure on \texorpdfstring{$\Corr(\C,\C_0)$}{Corr(C,C_0)}}

\begin{proposition} % prop.2.3.3
  Let $(\C,\C_0)$ be a geometric setup. 
  If $\C$ admits finite products, then $\Corr(\C,\C_0)^{\otimes}$ is a symmetric monoidal category.
\end{proposition}

\begin{proposition} % prop.2.3.5
  Let $(\C,\C_0)$ be a geometric setup. 
  The functor $(\C,\C_0) \to \Corr(\C,\C_0)^{\otimes}$ enhances to a functor 
  \begin{align*}
    \Corr^{\otimes} : \GS \to \CMon.
  \end{align*}
  If $\C \to \D$ preserves finite products, $\Corr(\C,\C_0)^{\otimes} \to \Corr(\D,\D_0)^{\otimes}$ is a symmetric monoidal functor.
  % Let $(\C,\C_0) \to (\D,\D_0)$ be a morphism of geometric setup.
  % If $\C$ and $\D$ admits finite products, $\Corr(\C,\C_0)^{\otimes} \to \Corr(\D,\D_0)^{\otimes}$ is a lax symmetric monoidal functor.
\end{proposition}

\begin{proposition} % prop.2.3.7
...
\begin{definition}[\cite{HM24} Definition 3.1.1] \label{HM24.def.3.1.1}
  ...
  A \textit{three-functor formalism} on $(\C,E)$ is a map of operads
  \begin{align*}
...
%   Thus a three-functor formalism on $(\C,E)$ is a lax symmetric monoidal functor
%   \begin{align*}
%     \rmD : \Corr(\C,E) \to \Cat.
%   \end{align*}
% \end{remark}
...
\begin{remark} \label{nota.D_apper_ast}
  Let $(\C,E)$ be a geometric setup, and let $ \rmD : \Corr(\C,E)^{\otimes} \to \Cat^{\times}$ be a three-functor formalism on $(\C,E)$.
  \begin{enumerate}
    \item By precomposing with the map of operads $(\C^{\myop})^{\coprod} \to \Corr(\C,E)^{\otimes}$, we obtain a map of operads $\rmD : (\C^{\myop})^{\coprod} \to \Cat^{\times}$.
    By \cite[Theorem 2.4.3.18]{HA}, it corresponds to a functor 
    \begin{align*}
      \rmD^{*} : \C^{\myop} \to \CMon.
    \end{align*}
    \item By precomposing with the map of operads $\C_{E} \to \Corr(\C,E)^{\otimes}$, we obtain a map of operads (?) 
    \begin{align*}
...
\begin{definition}[\cite{HM24} Definition 3.2.1] \label{HM24.def.3.2.1}
  Let $(\C,E)$ be a geometric setup.
  A \textit{six-functor formalism} on $(\C,E)$ is a three-functor formalism $\rmD : \Corr(\C,E)^{\otimes} \to \Cat^{\times}$ satisfying the following conditions:
  \begin{enumerate}
...
\begin{notation}
  Let $(\C,E)$ be a geometric setup, and let $D : \Corr(\C,E)^{\otimes} \to \Cat^{\times}$ be a six-functor formalism on $(\C,E)$.
  We will use the following notations:
  \begin{enumerate}
...
\subsection{Constructions of six-functor formalisms}
...
\begin{proposition}[\cite{HM24} Proposition 3.3.3] \label{HM24.prop.3.3.3}
  ...
  Then $\rmD$ can be extended to a three-functor formalism 
  \begin{align*}
    \rmD : \Corr(\C,E)^{\otimes} \to \Cat^{\times},
  \end{align*}
...
\begin{corollary}
  ...
  \end{enumerate}
  Then the three-functor formalism obtained by \cref{HM24.prop.3.3.3} is a six-functor formalism on $(\C,E)$. 
\end{corollary}

\subsection{Extensions of six-functor formalisms}
...
\begin{definition}[\cite{HM24} Definition 3.4.1] \label{HM24.def.3.4.1}
  Let $(\C,E)$ be a site setup, and let $\rmD : \Corr(\C,E)^{\otimes} \to \Cat^{\times}$ be a three-functor formalism on $(\C,E)$.
...
\begin{proposition}[\cite{HM24} Proposition 3.4.2] \label{HM24.prop.3.4.2}
  Let $(\C,E)$ be a subcanonical site setup, and let $\rmD : \Corr(\C,E)^{\otimes} \to \Cat^{\times}$ be a sheafy three-functor formalism.
  ...
  \begin{enumerate}
    ...
    \item The sheafy three-functor formalism $\rmD : \Corr(\C,E)^{\otimes} \to \Cat^{\times}$ can be uniquely extended to a sheafy three-functor formalism $\rmD' : \Corr(\X,E')^{\otimes} \to \Cat^{\times}$.
  \end{enumerate}
  Moreover, if $\rmD$ is a presentable sheafy six-functor formalism, so is $\rmD'$.
\end{proposition}
...
\begin{theorem}[\cite{HM24} Theorem 3.4.11] \label{HM24.thrm.3.4.11}
  Let $(\C,E)$ be a subcanonical site setup, and let $\rmD : \Corr(\C,E)^{\otimes} \to \Cat^{\times}$ be a sheafy presentable six-functor formalism.
  We let $\X:=\Shv(\C)$ denote the topos of sheaves on $\C$.
  ...
  \begin{enumerate}
    ...
    \item The sheafy presentable six-functor formalism $\rmD : \Corr(\C,E)^{\otimes} \to \Cat^{\times}$ can be uniquely extended to a sheafy six-functor formalism $\rmD' : \Corr(\X,E')^{\otimes} \to \Cat^{\times}$.
  \end{enumerate}
\end{theorem}
...
We fix a geometric setup $(\C,E)$ and a six-functor formalism $\rmD : \Corr(\C,E)^{\otimes} \to \Cat^{\times}$.
...
Note that the composition (especially the integral transformation) can be defined in the setting of three-functor formalisms $\rmD$.
However, there exists the right adjoint functor to it if $\rmD$ is a six-functor formalism.
...
We fix a geometric setup $(\C,E)$ and a three-functor formalism $\rmD : \Corr(\C,E)^{\otimes} \to \Cat^{\times}$.
...
\begin{definition}[\cite{Mann22} Definition A.5.6] \label{Mann.def.A.5.6}
  Let $(\C,E)$ be a geometric setup.
  A \textit{three-functor formalism} on $(\C,E)$ is a lax symmetric monoidal functor
  \begin{align*}
    D : \Corr(\C,E) \to \Cat.
  \end{align*}
...
\begin{remark}
  ...
  Let $ D : \Corr(\C,E)^{\otimes} \to \Cat^{\otimes}$ be a three-functor formalism on $(\C,E)$.
...
\begin{notation}
  Let $(\C,E)$ be a geometric setup, and let $ D : \Corr(\C,E)^{\otimes} \to \Cat^{\otimes}$ be a three-functor formalism on $(\C,E)$.
  We will use the following notations:
  \begin{enumerate}
...
\begin{definition}[\cite{Mann22} Definition A.5.7] \label{Mann.def.A.5.7}
  Let $(\C,E)$ be a geometric setup.
  A \textit{six-functor formalism} on $(\C,E)$ is a three-functor formalism $ D : \Corr(\C,E)^{\otimes} \to \Cat^{\otimes}$ on $(\C,E)$ which satisfies the following conditions:
  \begin{enumerate}
...
\begin{notation}
  Let $(\C,E)$ be a geometric setup, and let $D : \Corr(\C,E)^{\otimes} \to \Cat^{\otimes}$ be a six-functor formalism on $(\C,E)$.
  We will use the following notations:
  \begin{enumerate}
...
However, it is not obvious that $f_{!}$ is well-defined i.e. $f_{!}$ is independent on the choice of the factorization and it satisfies the required compatibilities for six-functor formalisms.
...
\begin{proposition}[\cite{Mann22} Proposition A.5.10] \label{Mann.prop.A.5.10}
  ...
  Then $D^{*}$ can be extended to a three-functor formalism 
  \begin{align*}
    D : \Corr(\C,E)^{\otimes} \to \Cat^{\otimes},
  \end{align*}
  on $(\C,E)$ such that for every morphism $j$ in $I$, $j_{!}$ is left adjoint to $j^{*}$, and for every morphism $g$ in $P$, $g_{!}$ is right adjoint to $g^{*}$.
  ...
  \end{enumerate}
  Then the above three-functor formalism $D$ is a six-functor formalism on $(\C,E)$. 
\end{proposition}
