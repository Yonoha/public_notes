...
% For example, Milnor defined defined the group $K_2(R)$.
...
\begin{itemize}
  ...
  \item $\Mon(\E)$ (resp. $\Grp(\E)$) denote the category of commutative monoid (resp. group) objects on $\C$ for a category $\C$.
\end{itemize}
...
% \begin{remark}
%   Let $\C$ be a category.
%   The construction $\C \mapsto \TwAr(\C)$ defines a functor $\TwAr : \Cat \to \Cat$.
%   It preserves all limits and colimits since limits and colimits can be computed levelwise.
% \end{remark}
...
\section{Proofs in \Cref{sec.localization_properties} and \Cref{sec.algebraic_k_theory}}
